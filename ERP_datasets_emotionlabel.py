...
df_emotion = pd.read_excel('./emotion_reorder.xlsx', sheet_name=0, header=0) 
intent2emotion = defaultdict()
for index, row in df_emotion.iterrows():
    trial_id, emotion_id = row['trial_ID'], row['trial_ID_emotion']
    intent2emotion[trial_id] = emotion_id
    print(trial_id, emotion_id)
...
if READ_FLAG == 1:
    ...
    for sub in train_subs: 
        ...
        for j in range(nSamples):
            ...
            tmp_label = df_data.iloc[j]['trial_ID']
            sub_label = intent2emotion[tmp_label]
            emotion = sub_label.split('_')[-1]
            print(tmp_label, sub_label, emotion)
            if emotion=='JG':
                temp_labels[j] = 2
...
feature_save_path = './datasets_cross_sublabel/datasets_cross_17s_emotion/'
