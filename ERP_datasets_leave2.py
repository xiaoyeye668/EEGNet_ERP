...
# 初始化两个变量分别存储每个被试的脑电数据和对应的类别标签
# 28 - n_subjects, 14 - n_freqs， 500 - n_times
fs = 500
...
if READ_FLAG == 1:
    sub_num = 0
    for ii in range(n_train_subjects):
        sub = train_sub_lists[ii]
        temp_data = []
        temp_labels = []
    ...
    #处理test dataset
    sub_num = 0
    for ii in range(n_test_subjects):
        sub =  test_sub_lists[ii]
        temp_data = []
        temp_labels = []
        data_path = preprocessed_path + '/cleandata_' + sub + '.mat' 
        chas_arr,fsample_arr, features_arr, label_arr = read_from_mat(data_path)
        ...
        for j in range(nSamples):
            ...
            temp_labels[j] = event_dic[str(label_arr[j][0])]
        if sub_num == 0:
            X_test = np.array(temp_data)*1000
            y_test = np.array(temp_labels)
        else:
            #X = np.concatenate((X,np.array(temp_data)), axis=0)
            X_test = np.concatenate((X_test, np.array(temp_data)*1000), axis=0)
            y_test = np.concatenate((y_test, np.array(temp_labels)), axis=0)
        sub_num += nSamples
        print(sub_num, X_test.shape,y_test.shape)
...
feature_save_path = './datasets_cross_new/datasets_cross_17s_leave2/'
