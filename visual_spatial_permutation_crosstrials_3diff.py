...
def tfr_byclass(class_out_data, samples):
    ...
    for i in range(len(class_out_data)):
        ...
        data = class_out_data[i,:,:,1:2].reshape((1,1,samples)) #(n_epochs, n_chans, n_times)
        # 设定一些时频分析的参数
        # 频段选取0.1-32Hz
...
plot_tfr_diff_results(tfr1, tfr2, freqs, times, 
                      p=0.01, clusterp=0.05, threshold=500.0,clim=[-2, 2])
plot_tfr_diff_results(tfr3, tfr2, freqs, times, 
                      p=0.01, clusterp=0.05, threshold=500.0,clim=[-2, 2])
plot_tfr_diff_results(tfr3, tfr1, freqs, times, 
                      p=0.01, clusterp=0.05, threshold=500.0,clim=[-2, 2])
#plot_tfr_diff_results(tfr1, tfr2, freqs, times, 
